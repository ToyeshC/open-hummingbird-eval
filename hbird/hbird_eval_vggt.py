...
# Import the VGGT-specific data module if it exists, otherwise use the regular one
try:
    from hbird.data.mvimgnet_data_vggt import MVImgNetDataModule as MVImgNetDataModuleVGGT
    USE_VGGT_DATA_MODULE = True
except ImportError:
    USE_VGGT_DATA_MODULE = False
    print("Warning: mvimgnet_data_vggt not found, using regular MVImgNetDataModule")
...
class HbirdEvaluation():
    ...
    def create_memory(self, train_loader, num_classes, eval_spatial_resolution):
        ...
        with torch.no_grad():
            for j in tqdm(range(self.augmentation_epoch), desc='Augmentation loop', mininterval=10):
                for i, batch in enumerate(tqdm(train_loader, desc='Memory Creation loop', mininterval=10)):
                    if isinstance(batch, dict):
                        # VGGT multi-view format: views is (B, S, C, H, W) where S is sequence length
                        x = batch["views"].to(self.device)
                        cameras = batch.get("camera")
                        y = batch.get("mask")
                        if y is None:
                            raise ValueError("MVImgNet dataset must provide masks when building the memory.")
                        y = y.to(self.device)
                    else:
                        x, y = batch
                        x = x.to(self.device)
                        y = y.to(self.device)
                        cameras = None
                        # For non-dict batches, ensure x has sequence dimension for VGGT
                        if x.dim() == 4:  # (B, C, H, W) -> add sequence dimension
                    ...
                    features, _ = self.feature_extractor.forward_features(x, camera=cameras) # features of shape (BS, PS, D)
                    # Move features to CPU immediately to free GPU memory
                    features = features.cpu()
                    
                    # Move y to CPU for processing (no need to keep on GPU)
                    y = y.cpu()
                    ...
                    if self.memory_size is None:
                        ...
                        feature_memory.append(normalized_features.detach())
                        label_memory.append(label.detach())
                    else:
                        # Memory Size is bounded so we need to select/sample some features only
                        # Move patchified_gts to CPU for sampling
                        patchified_gts = patchified_gts.cpu()
                        ...
                        sampled_indices = sampled_indices  # Already on CPU
                        ...
                        self.feature_memory[idx:idx+normalized_sampled_features.size(0)] = normalized_sampled_features.detach()
                        self.label_memory[idx:idx+label_hat.size(0)] = label_hat.detach()
                        idx += normalized_sampled_features.size(0)
                    
                    # Clear GPU cache periodically during memory creation
                    if (i + 1) % 50 == 0:
    ...
    def sample_features(self, features, pathified_gts):
        ...
        Assumes features and pathified_gts are on CPU.

        Args:
            features (torch.Tensor): Feature tensor of shape (BS, PS, D) on CPU.
            pathified_gts (torch.Tensor): Patchified ground truth tensor on CPU.
    ...
    def get_class_frequency(self, gt):
        ...
        Works on CPU tensors.

        Args:
            gt (torch.Tensor): Ground truth tensor on CPU.

        Returns:
            torch.Tensor: Class frequency tensor on CPU.
        """
    ...
    def evaluate(self, val_loader, eval_spatial_resolution, return_knn_details=False, ignore_index=255, aggregate_across_classes=True, return_full_label_hats=False):
        ...
        Memory-efficient version that processes metrics incrementally to avoid OOM.
        ...
        # Only accumulate if we need to return full results
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(val_loader, desc='Evaluation loop')):
                if isinstance(batch, dict):
                    # VGGT multi-view format: views is (B, S, C, H, W)
                    x = batch["views"].to(self.device)
                    cameras = batch.get("camera")
                    masks = batch.get("mask")
                    if masks is None:
                        raise ValueError("MVImgNet validation batches must provide masks.")
                    y = masks.to(self.device)
                    # Masks are aligned with query view (first in sequence)
                    if y.dim() == 5:  # (B, S, C, H, W) -> take first view
                        y = y[:, 0]
                    if y.dim() == 3:
                        y = y.unsqueeze(1)
                else:
                ...
                # Handle multi-view input: x is (B, S, C, H, W) for VGGT
                if x.dim() == 5:
                    query_view = x[:, 0]  # Take first view as query
                ...
                # Extract features
                features, _ = self.feature_extractor.forward_features(x, camera=cameras)
                
                # Move to CPU immediately to free GPU memory
                features = features.cpu()
                
                # Find nearest neighbors
                q = features  # Use features directly, no need to clone
                ...
                # Reshape and upsample predictions
                ...
                # Move to CPU and process immediately to avoid accumulating on GPU
                cluster_map = cluster_map.cpu()
                y = y.cpu()
                
                # Process metrics incrementally to avoid OOM
                valid_idx = y != ignore_index
                if valid_idx.any():
                    valid_target = y[valid_idx]
                    valid_cluster_maps = cluster_map[valid_idx]
                    metric.update(valid_target, valid_cluster_maps)
                
                # Only accumulate if needed for return
                if return_full_label_hats:
            ...
            if return_full_label_hats:
                full_label_hats = torch.cat(label_hats) if label_hats else None
                full_lables = torch.cat(lables) if lables else None
                return full_label_hats, full_lables, jac, input_lis
            
            if return_knn_details:
                knns = torch.cat(knns) if knns else None
                knns_labels = torch.cat(knns_labels) if knns_labels else None
                knns_ca_labels = torch.cat(knns_ca_labels) if knns_ca_labels else None
...
def hbird_evaluation(model, d_model, patch_size, dataset_name:str, data_dir:str, batch_size=64, input_size=224, 
    ...
    if dataset_name == "voc":
        ...
        # For VGGT, use the data module that supports multi-view sequences with camera metadata
        # The MVImgNetDataModule supports this via sequence_length parameter
        # and returns views as (B, S, C, H, W) with camera metadata when raw_data_dir is available
        # Try to infer raw data directory for camera metadata (COLMAP sparse reconstructions)
        import pathlib
        data_path = pathlib.Path(data_dir)
        raw_data_dir = None
        # Check if raw mvimgnet directory exists (for camera metadata)
        if (data_path.parent / "mvimgnet").exists():
            raw_data_dir = str(data_path.parent / "mvimgnet")
            print(f"[VGGT] Found raw MVImgNet directory for camera metadata: {raw_data_dir}")
        elif (data_path / ".." / "mvimgnet").resolve().exists():
            raw_data_dir = str((data_path / ".." / "mvimgnet").resolve())
            print(f"[VGGT] Found raw MVImgNet directory for camera metadata: {raw_data_dir}")
        
        # Create dataset with multi-view support
        # Check if MVImgNetDataModule supports raw_data_dir parameter
        dataset_kwargs = {
            "data_dir": data_dir,
            ...
            "num_workers": num_workers,
            "return_masks": True,
            "sequence_length": sequence_length,  # VGGT needs multi-view sequences
        }
        # Add raw_data_dir if the data module supports it (for camera metadata)
        if raw_data_dir is not None:
            # Check if MVImgNetDataModule accepts raw_data_dir
            import inspect
            sig = inspect.signature(MVImgNetDataModule.__init__)
            if "raw_data_dir" in sig.parameters:
                dataset_kwargs["raw_data_dir"] = raw_data_dir
        
        dataset = MVImgNetDataModule(**dataset_kwargs)
        dataset.setup()
        # The default ignore_index=-1 is used to not ignore any class
        print(f"[VGGT] Using sequence_length={sequence_length} for multi-view feature extraction")
        if raw_data_dir:
            print(f"[VGGT] Camera metadata will be used from: {raw_data_dir}")
        else:
            print(f"[VGGT] Warning: No raw MVImgNet directory found, camera metadata unavailable")
    else:
    ...
    if dataset_name == "mvimgnet":  # evaluation is done on a specific bin for all classes
        ...
        # Get raw_data_dir from the dataset if available (for camera metadata)
        # The MVImgNetDataModule may or may not have raw_data_dir attribute
        raw_data_dir_for_val = getattr(dataset, 'raw_data_dir', None)
        if raw_data_dir_for_val is None:
            # Try to infer it the same way we did for training
            import pathlib
            data_path = pathlib.Path(data_dir)
            if (data_path.parent / "mvimgnet").exists():
                raw_data_dir_for_val = str(data_path.parent / "mvimgnet")
            elif (data_path / ".." / "mvimgnet").resolve().exists():
                raw_data_dir_for_val = str((data_path / ".." / "mvimgnet").resolve())
        ...
        for val_bin in val_bins:
            # Use same configuration for validation bins
            val_dataset_kwargs = {
                "data_dir": data_dir,
                "train_bins": None,
                "val_bins": [val_bin],
            ...
            # Add raw_data_dir if supported
            if raw_data_dir_for_val is not None:
                import inspect
                sig = inspect.signature(MVImgNetDataModule.__init__)
                if "raw_data_dir" in sig.parameters:
                    val_dataset_kwargs["raw_data_dir"] = raw_data_dir_for_val
            
            val_bin_dataset = MVImgNetDataModule(**val_dataset_kwargs)
