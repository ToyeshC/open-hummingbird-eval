...
TRAIN_BINS_SETS = set()
lengths = range(1, 8)
all_training_bins = [0, 15, 30, 45, 60, 75, 90]
for r in lengths:
    perms = permutations(all_training_bins, r)
    sets_r = {frozenset(p) for p in perms}
    TRAIN_BINS_SETS.update(sets_r)
...
def main(args):
    ...
    for train_bins in tqdm(list(TRAIN_BINS_SETS), desc="Running experiments", mininterval=10):
        train_bins = list(train_bins)
        ...
        for i in range(len(all_training_bins)):
            with open(RESULTS_PATH, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([
                    JOB_ID, model_label, args.class_num, train_str, all_training_bins[i], 
                    hbird_miou[i][0], hbird_miou[i][1], np.mean(hbird_miou[i]),
                    args.d_model, args.input_size, args.patch_size])
                print(f"train_bins : {train_str} , 0 preds acc : {round(hbird_miou[i][0], 2)}, 1 preds acc : {round(hbird_miou[i][1],2)}") 
